...
```plantuml
@startuml
rectangle "Interne Analyse" {
  rectangle "Stärken (S)" as S
  rectangle "Schwächen (W)" as W
}
rectangle "Externe Analyse" {
  rectangle "Chancen (O)" as O
  rectangle "Risiken (T)" as T
}
S -right-> O : SO-Strategie
S -right-> T : ST-Strategie
W -right-> O : WO-Strategie
W -right-> T : WT-Strategie
@enduml
```
